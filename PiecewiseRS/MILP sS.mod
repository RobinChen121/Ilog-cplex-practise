/*********************************************
 * OPL 12.7.0.0 Model
 * Author: Administrator
 * Creation Date: 2018年11月13日 at 上午11:26:31
 * Description: implement the joint MILP model
 *********************************************/

 //parameters
int nbmonths=...;
range months=1..nbmonths;
float fc=...;
float h=...;
float p=...;
float meandemand[months]=...;
float std_demand[months]=...;
float initialstock=...;

int nbpartitions=...;
range partitions=1..nbpartitions;
float means[partitions]=...;
float prob[partitions]=...;
float error=...;

//variables
dvar float stock[0..nbmonths];
dvar float+ stockhlb[0..nbmonths];
dvar float+ stockplb[0..nbmonths];
dvar boolean purchase[months];
dvar boolean P[months][months];
dvar float I0;

dvar float stock2[0..nbmonths];
dvar float+ stockhlb2[0..nbmonths];
dvar float+ stockplb2[0..nbmonths];
dvar boolean purchase2[months];
dvar boolean P2[months][months];
dvar float I02;

//float mean_matrix[i in months, j in months] = sum(m in i..j) meandemand[m];
float std_matrix[i in months, j in months] = sqrt(sum(m in i..j) pow(std_demand[m],2));

//objective function
minimize sum(t in months)( fc*purchase[t]+h*stockhlb[t]+p*stockplb[t]) + sum(t in 1..nbmonths)( fc*purchase2[t]+h*stockhlb2[t]+p*stockplb2[t]);

// expressions
dexpr float Cx = sum(t in months)( fc*purchase[t]+h*stockhlb[t]+p*stockplb[t]) - 0*stock[0] + 0*stock[nbmonths];
dexpr float Gy = h*stockhlb2[1]+p*stockplb2[1] + sum(t in 2..nbmonths)( fc*purchase2[t]+h*stockhlb2[t]+p*stockplb2[t])- 0*stock2[0] + 0*stock2[nbmonths];
dexpr float partialGy = sum(t in 2..nbmonths)( fc*purchase2[t]+h*stockhlb2[t]+p*stockplb2[t]);

//constraints
subject to{
 stock[0]==I0;
 stock2[0]==I02;
 stockhlb[0]==maxl(stock[0],0);
 stockplb[0]==maxl(-stock[0],0);
 stockhlb2[0]==maxl(stock2[0],0);
 stockplb2[0]==maxl(-stock2[0],0);
 
 purchase[1] == 1; // x1[1] = 1
 purchase2[1] == 0; // x2[1] = 0
 
 I0 >= I02;
 I0 == stock[1] + meandemand[1];
 
 sum(t in months)( fc*purchase[t]+h*stockhlb[t]+p*stockplb[t]) - 0*stock[0] + 0*stock[nbmonths] == h*stockhlb2[1]+p*stockplb2[1] + sum(t in 2..nbmonths)( fc*purchase2[t]+h*stockhlb2[t]+p*stockplb2[t])- 0*stock2[0] + 0*stock2[nbmonths];

 forall(t in months) {
   		purchase[t] == 0 => stock[t]+meandemand[t]-stock[t-1] == 0;  
   		purchase2[t] == 0 => stock2[t]+meandemand[t]-stock2[t-1] == 0;  
 }
   		   
forall(t in months){
  		stock[t]+meandemand[t]-stock[t-1]>=0;
  		stock2[t]+meandemand[t]-stock2[t-1]>=0;
  }  		
  
forall (t in months){
  		sum(j in 1..t)P[j][t]==1;
  		sum(j in 1..t)P2[j][t]==1;
  }  		
 
forall (t in months, j in 1..t){
  		P[j][t]>=purchase[j]-sum(k in j+1..t)purchase[k];
  		P2[j][t]>=purchase[j]-sum(k in j+1..t)purchase2[k];
  }  		
  
forall (t in months){
   		sum(k in 1..t) purchase[k] == 0 => P[1][t] == 1;  
   		sum(k in 1..t) purchase2[k] == 0 => P2[1][t] == 1;
  }
  
/**
* Original formulation as in (Rossi et al., 2015)    
*/ 
forall(t in months, p in partitions){
  stockhlb[t]>=sum(k in 1..p)prob[k]*stock[t]-sum(j in 1..t)(sum(k in 1..p)prob[k]*means[k]*std_matrix[j][t]*P[j][t]) + (sum(j in 1..t) error*std_matrix[j][t]*P[j][t]);
  stockhlb2[t]>=sum(k in 1..p)prob[k]*stock2[t]-sum(j in 1..t)(sum(k in 1..p)prob[k]*means[k]*std_matrix[j][t]*P2[j][t]) + (sum(j in 1..t) error*std_matrix[j][t]*P2[j][t]);
}  

  forall(t in months) {
    stockhlb[t] >= (sum(j in 1..t) error*std_matrix[j][t]*P[j][t]);
    stockhlb2[t] >= (sum(j in 1..t) error*std_matrix[j][t]*P2[j][t]);
  }

    
/**
* Original formulation as in (Rossi et al., 2015)    
*/ 
forall(t in months, p in partitions){
  stockplb[t]>=-stock[t]+sum(k in 1..p)prob[k]*stock[t]-sum(j in 1..t)(sum(k in 1..p)prob[k]*means[k]*std_matrix[j][t]*P[j][t]) + (sum(j in 1..t) error*std_matrix[j][t]*P[j][t]);
  stockplb2[t]>=-stock2[t]+sum(k in 1..p)prob[k]*stock2[t]-sum(j in 1..t)(sum(k in 1..p)prob[k]*means[k]*std_matrix[j][t]*P2[j][t]) + (sum(j in 1..t) error*std_matrix[j][t]*P2[j][t]);
}  
 
  forall(t in months) {
    stockplb[t] >= - stock[t] + (sum(j in 1..t) error*std_matrix[j][t]*P[j][t]);
    stockplb2[t] >= - stock2[t] + (sum(j in 1..t) error*std_matrix[j][t]*P2[j][t]);
  }
  
}

execute {
  if (cplex.getCplexStatus() == 1) {
    writeln('Cx is ', Cx);
    writeln('Gy is ', Gy);
    writeln('partial Gy is ', partialGy)
  } 
}